...
class MLPRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 5000):
        ...
        self.W1 = nodes.ValueNode('W1')
        self.b1 = nodes.ValueNode('b1')
        self.W2 = nodes.ValueNode('W2')
        self.b2 = nodes.ValueNode('b2')
        self.L = nodes.AffineNode(self.W1, self.x, self.b1, 'L')
        self.h = nodes.TanhNode(self.L, 'h')
        self.prediction = nodes.AffineNode(self.W2, self.h, self.b2, 'prediction')
        self.objective = nodes.SquaredL2DistanceNode(self.prediction, self.y, 'objective')
