...
class AffineNode(object):
    ...
    def backward(self):
        self.W.d_out += (self.d_out.reshape((self.d_out.size, 1)) @ self.x.out.reshape((self.x.out.size, 1)).T).reshape(self.W.d_out.shape)
        d_x = self.W.out.T * self.d_out if self.d_out.shape == () else self.W.out.T @ self.d_out
        self.x.d_out += d_x
        self.b.d_out += self.d_out
        return self.d_out
