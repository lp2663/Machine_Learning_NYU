...
class RidgeRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, l2_reg=1, step_size=.005,  max_num_epochs = 5000):
        ...
        self.h = nodes.L2NormPenaltyNode(l2_reg, self.w, 'h')
        self.g = nodes.SquaredL2DistanceNode(self.prediction, self.y, 'g')
        self.objective = nodes.SumNode(self.h, self.g, 'objective')
        # Build computation graph
        self.graph = graph.ComputationGraphFunction([self.x],
                                                    [self.y],
                                                    [self.w, self.b],
                                                    self.prediction,
                                                    self.objective)
